...
const CategoryCard = ({ category }: CategoryCardProps) => {
  const colorMap: Record<string, string> = {
    indigo: "hover:border-indigo-200 hover:shadow-indigo-500/10 bg-white text-indigo-600 hover:text-indigo-700",
    blue: "hover:border-blue-200 hover:shadow-blue-500/10 bg-white text-blue-600 hover:text-blue-700",
    rose: "hover:border-rose-200 hover:shadow-rose-500/10 bg-white text-rose-600 hover:text-rose-700",
    fuchsia: "hover:border-fuchsia-200 hover:shadow-fuchsia-500/10 bg-white text-fuchsia-600 hover:text-fuchsia-700",
    emerald: "hover:border-emerald-200 hover:shadow-emerald-500/10 bg-white text-emerald-600 hover:text-emerald-700",
    amber: "hover:border-amber-200 hover:shadow-amber-500/10 bg-white text-amber-600 hover:text-amber-700",
    sky: "hover:border-sky-200 hover:shadow-sky-500/10 bg-white text-sky-600 hover:text-sky-700",
  };
  ...
  return (
    <motion.div
      ...
      transition={{ type: "spring" as any, stiffness: 300, damping: 15 }}
      className={`group relative h-64 w-full overflow-hidden rounded-2xl border p-6 shadow-sm transition-all duration-300 hover:shadow-2xl ${styles}`}
    >
      {/* Default State */}
      <motion.div
      ...
      >
        <div className={`mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-white/20 ${styles.split(' ').slice(4).join(' ')}`}>
          <category.icon className="h-8 w-8" />
        </div>
        <motion.h3
          className={`text-center font-display text-xl font-bold ${styles.split(' ').slice(4).join(' ')}`}
      ...
      {/* Hover State */}
      <div className={`absolute inset-0 flex flex-col items-center justify-center p-6 opacity-0 backdrop-blur-sm transition-opacity duration-300 group-hover:opacity-100 bg-white/90`}>
  ...
  return (
    <div className="w-full py-12">
      ...
      <motion.div
        ...
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.5, delay: 0.3, type: "spring" as any, stiffness: 100 }}
